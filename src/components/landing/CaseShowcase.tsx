...
const cases: Case[] = [
  {
    ...
    after:  { label: "학습 후 AI", example: "앗 고마워유~ 🐔 우리 후라이드 맛있쥬? 사장이 매일 새벽에 직접 염지하는 비법이 있거든유 ❤️ 담에 오시면 양념반후라이드반 드셔봐유~ 우리 집 특제 마늘소스 찍어 먹으면 진짜 눈물남유 😭" },
    level: 2,
    levelLabel: "+말투 학습",
    category: "역할 학습",
  ...
  },
  {
    ...
    before: { label: "학습 전", example: "네, 다이어트에 효과적입니다! 많은 분들이 체중 감소를 경험하셨어요! (← 식약법 위반 가능)" },
    after:  { label: "학습 후", example: "식약처 인증 문구 외 효능 안내는 법적으로 어렵습니다 😊 다만 이 제품은 식약처 인증 '건강기능식품'으로, 체지방 감소에 도움을 줄 수 있음이 인정되었어요. 인증서 확인해드릴까요? 교환/환불은 수령 후 7일 이내 가능합니다!" },
    level: 3,
    levelLabel: "+규정 학습",
    category: "지식 학습",
  ...
  },
  {
    ...
    after:  { label: "학습 후", example: "아이가 파닉스 단계시군요! 저희 레벨2-B반이 딱 맞아요 — 원어민 선생님이 게임 기반으로 가르치셔서 아이들이 정말 좋아해요. 주 3회(월수금) 3시 반인데, 무료 레벨테스트 한번 받아보시겠어요? 15분이면 끝나고, 아이 수준에 맞는 커리큘럼을 바로 안내드릴 수 있어요 😊" },
    level: 3,
    levelLabel: "+전문지식",
    category: "지식 학습",
  ...
  },
  {
    ...
    after:  { label: "학습 후", example: "비 오는 교토의 오후, 처마 끝에서 떨어지는 빗방울을 세다가 라떼 한 잔에 시간이 멈추다.\n\n오늘의 원두는 에티오피아 예가체프 — 꽃향이 빗소리와 참 잘 어울려요. ☔🍵\n\n#교토카페일상 #雨の日カフェ #예가체프 #카페시간 #교토여행" },
    level: 2,
    levelLabel: "+톤 학습",
    category: "패턴 학습",
  ...
  },
  {
    ...
    before: { label: "학습 전", example: "안녕하세요. 오늘은 M4 칩에 대해 알아보겠습니다. M4 칩은 Apple이 개발한..." },
    after:  { label: "학습 후", example: "야 이거 진짜 미쳤거든? M4 칩 벤치 돌려봤는데 — 시네벤치 멀티가 M3 대비 42% 올라갔거든? 근데 재밌는 게 발열은 오히려 줄었어. 이게 가능한 이유가 3nm 공정 개선인데, 쉽게 말하면 같은 면적에 트랜지스터를 더 때려박았는데 전기는 덜 먹는 거야." },
    level: 3,
    levelLabel: "+말투+구성",
    category: "패턴 학습",
  ...
  },
  {
    ...
    before: { label: "학습 전", example: "34평 아파트입니다. 역에서 가깝습니다. 관심 있으시면 연락주세요." },
    after:  { label: "학습 후", example: "실평수 28평 남향 4Bay, 채광 끝내줍니다. 2호선 역삼역 도보 7분, 학군은 OO초 300m·OO중 500m. 2024년 풀 리모델링 완료(주방·욕실), 최근 동일평형 실거래 9.2억. 관리비 월 28만원 (난방 포함). 주차 세대당 1.5대." },
    level: 2,
    levelLabel: "+패턴 학습",
    category: "패턴 학습",
  ...
  },
  {
    ...
    before: { label: "학습 전", example: "API는 Application Programming Interface의 약자입니다. 프로그램 간 통신을 위한..." },
    after:  { label: "학습 후", example: "API는 식당의 메뉴판이에요. 주방에 직접 들어가지 않아도 메뉴판만 보면 주문할 수 있죠? 여기서 자주 헷갈리시는 부분! — REST API는 '메뉴판 양식'이고, JSON은 '주문서 형식'이에요. 실습으로 카카오 날씨 API를 직접 호출해볼 건데, 3줄이면 돼요." },
    level: 3,
    levelLabel: "+설명법 학습",
    category: "패턴 학습",
  ...
  },
  {
    ...
    before: { label: "Memory 없이", example: "모든 고객에게 \"안녕하세요! 좋은 하루 되세요~\" 같은 톤 → 매번 수정 요청 30분 낭비" },
    after:  { label: "Memory 쌓인 후", example: "A고객(법률사무소): \"전문성과 신뢰를 전하는 콘텐츠를 준비했습니다\"\nB고객(네일샵): \"이거 진짜 손톱이 보석 되는 느낌이에요~✨\"\n→ 고객별 톤 전환이 자동, 수정 요청 90% 감소" },
    level: 3,
    levelLabel: "+맥락 축적",
    category: "기억 학습",
  ...
  },
  {
    ...
    before: { label: "학습 전", example: "고혈압에 대해 설명해드리겠습니다. 고혈압은 수축기 혈압이 140mmHg 이상인 상태로..." },
    after:  { label: "학습 후", example: "혈압약(아침 OO정)은 매일 같은 시간에 꼭 드세요!\n\n⚠️ 이런 증상 있으면 즉시 내원:\n• 갑자기 심한 두통 + 구토\n• 가슴 통증 + 호흡곤란\n\n일상 팁: 어지러우면 천천히 일어나시고, 나트륨은 하루 2g 이하로. 다음 외래 일정은 4/15(화)입니다." },
    level: 3,
    levelLabel: "+의료 지식",
    category: "전문 학습",
  ...
  },
  {
    ...
    icon: Camera,
    who: "쇼핑몰 대표",
    title: "상품 사진을 AI로 무한 생성",
    methods: ["knowledge", "fine_tuning"],
    whatFed: [
      "상품 사진 25장 (흰배경+실사용)",
      "미니멀 스튜디오 스타일 참조 10장",
    ...
    howTaught: "상품 사진 25장으로 LoRA 학습 → 프롬프트만 바꾸면 무한 컨셉 변경",
    learningInsight: "흰배경만 넣으면 배경 분리가 안 됨. 다양한 각도+조명의 사진 20장이 최소. 학습 후 '크리스마스 분위기'만 입력하면 10초에 20장",
    before: { label: "기존 방식", example: "스튜디오 촬영 50만원 → 촬영 반나절 → 보정 3일 → 컨셉당 5장이 전부" },
    after:  { label: "AI 생성", example: "LoRA 학습 1회(20분) → '카페 배경, 자연광' 프롬프트 → 10초에 20장 생성. 크리스마스, 발렌타인, 여름 등 시즌별 무한. 연간 촬영비 90% 절감" },
    level: 3,
    levelLabel: "+이미지 생성",
    category: "이미지 생성",
    gradient: "from-fuchsia-400 to-pink-500",
    templateSlug: "image-gen-product-photo",
  },
  {
    id: 18,
    icon: Mic,
    who: "유튜브 크리에이터",
    title: "내 목소리로 AI 나레이션 자동 생성",
    methods: ["knowledge", "fine_tuning"],
    whatFed: [
      "내 목소리 녹음 5분 (조용한 환경)",
      "동일 대본 텍스트",
    ...
    howTaught: "깨끗한 녹음 5분 + 대본 텍스트 → TTS 모델 학습 → 텍스트만 입력하면 내 목소리로 자동 생성",
    learningInsight: "배경 소음이 있으면 음질이 급격히 떨어짐. 조용한 방에서 마이크 20cm 거리로 3분만 녹음하면 충분. 감정 변화도 학습됨",
    before: { label: "수동 녹음", example: "10분 나레이션 → 녹음 30분 + NG 재녹음 + 편집 1시간 = 총 2시간" },
    after:  { label: "AI 음성", example: "블로그 글 복붙 → 30초 만에 내 목소리로 10분 나레이션 생성. 속도·감정·강조 조절 가능. 주 5개 영상 → 나레이션 시간 95% 절감" },
    level: 3,
    levelLabel: "+음성 생성",
    category: "음성 학습",
    gradient: "from-violet-400 to-purple-500",
    templateSlug: "voice-clone-creator",
  },
  {
    id: 19,
    icon: Database,
    who: "IT 스타트업 CTO",
    title: "사내 문서 1만 건을 AI가 즉시 검색",
    methods: ["knowledge", "rag"],
    whatFed: [
      "사내 위키 문서 3,000건 (Notion)",
      "슬랙 대화 기록 2년치",
      "기술 블로그 포스팅 200건",
      "온보딩 매뉴얼 + FAQ 500건",
    ],
    howTaught: "전체 문서를 벡터DB에 인덱싱 → 직원이 자연어로 질문하면 관련 문서를 찾아서 답변 생성",
    learningInsight: "chunk 크기가 핵심. 너무 크면(2000토큰) 노이즈, 너무 작으면(100토큰) 맥락 손실. 500토큰 + overlap 50이 최적. 신입 온보딩 시간 60% 단축",
    before: { label: "기존 검색", example: "슬랙에서 '배포 절차' 검색 → 100개 결과 → 최신 문서 찾기 30분 → 결국 선배에게 물어봄" },
    after:  { label: "RAG 검색", example: "'스테이징 배포 어떻게 해?' → AI가 최신 배포 가이드 + 관련 PR 링크 + 주의사항까지 5초 만에 답변. 신입 온보딩 질문 70% 자동 해결" },
    level: 4,
    levelLabel: "+RAG 구축",
    category: "지식 학습",
    gradient: "from-cyan-400 to-blue-500",
    templateSlug: "rag-enterprise-doc-system",
  },
  {
    id: 20,
    icon: BarChart3,
    who: "개발팀 리드",
    title: "코드 리뷰를 AI가 자동으로",
    methods: ["knowledge", "system_prompt", "rag"],
    whatFed: [
      "팀 코딩 컨벤션 문서",
      "기존 코드 리뷰 코멘트 2,000건",
      "레포지토리 코드베이스 전체",
    ...
    howTaught: "코딩 컨벤션 + 기존 리뷰 패턴을 학습시키고, PR이 올라오면 자동으로 리뷰 코멘트 생성",
    learningInsight: "AI가 놓치기 쉬운 건 비즈니스 로직 오류. 순수 코드 품질(네이밍, 중복, 보안) 리뷰는 95% 정확하지만, 비즈니스 로직은 사람이 봐야 함",
    before: { label: "수동 리뷰", example: "PR 30개/주 → 시니어 개발자 리뷰 평균 2시간/PR → 병목 → 머지까지 평균 3일" },
    after:  { label: "AI 리뷰", example: "PR 올리면 5분 내 AI 1차 리뷰 완료. 네이밍 규칙, 보안 취약점, 미사용 코드 자동 탐지. 시니어는 비즈니스 로직만 집중 → 머지까지 평균 4시간" },
    level: 3,
    levelLabel: "+코드 분석",
    category: "데이터 학습",
    gradient: "from-indigo-400 to-blue-600",
    templateSlug: "code-gen-assistant",
  },
  {
    id: 21,
    icon: Video,
    who: "마케팅 에이전시",
    title: "프롬프트만으로 15초 광고 영상 생성",
    methods: ["knowledge", "few_shot"],
    whatFed: [
      "브랜드 가이드라인 (색상, 폰트, 톤)",
      "기존 광고 영상 참조 10개",
    ...
    howTaught: "참조 영상 + 브랜드 가이드라인 + 스토리보드를 입력하면 AI가 15초 시네마틱 영상 자동 생성",
    learningInsight: "참조 영상 없이 텍스트만 주면 품질이 50%. 3~5개 참조 영상을 주면 브랜드 톤이 일관됨. 씬별 키프레임 지정이 품질을 결정",
    before: { label: "외주 제작", example: "영상 1개 → 기획 1주 + 촬영 1일 + 편집 2주 → 비용 300~500만원 → 수정 3회까지" },
    after:  { label: "AI 생성", example: "프롬프트+참조 입력 5분 → 15초 영상 3분 생성 → 수정 무제한 → 비용 0원. A/B 테스트용 3개 버전도 10분이면 완성" },
    level: 3,
    levelLabel: "+영상 생성",
    category: "패턴 학습",
    gradient: "from-red-400 to-orange-500",
    templateSlug: "video-gen-marketing",
  },
  {
    id: 14,
    icon: AlertTriangle,
